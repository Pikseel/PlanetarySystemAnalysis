...
/**
 * Manages a planetary system as a tree structure.
 */
public class PlanetarySystem {
    private Node root;

    /**
     * Creates a planetary system with a star at the root.
     * @param starName name of the star
     * @param temperature in Kelvin
     * @param pressure in Pascals
     * @param humidity percentage (must be 0 for star)
    ...
    /**
     * Adds a planet as a child of the star.
     * @param planetName name of the planet
     * @param parentName name of the parent (must be star)
     * @param temperature in Kelvin
     * @param pressure in Pascals
    ...
    public void addPlanet(String planetName, String parentName, double temperature, double pressure, double humidity, double radiation) {
        if (!root.getName().equals(parentName)) {
            throw new IllegalArgumentException("Planets can only be added to the star.");
        }
     ...
     * @param parentName name of the parent (must be a planet)
     * @param temperature in Kelvin
     * @param pressure in Pascals
     * @param humidity percentage
     * @param radiation in Sieverts
    ...
    /**
     * Finds all nodes with radiation above the threshold.
     ...
     * @return stack representing the path, or null if not found
     */
    ...
    private void printNode(Node node, int level) {
        ...
        System.out.printf("%s%s (%s): %.2f Kelvin, %.2f Pascals, %.2f%% humidity, %.2f Sieverts%n",
                indent, node.getName(), node.getType(), data.getTemperature(), data.getPressure(),
                data.getHumidity(), data.getRadiation());
    ...
    public void printMissionReport(String nodeName) {
        ...
        if (node == null) {
            ...
            System.out.printf("%s (%s): %.2f Kelvin, %.2f Pascals, %.2f%% humidity, %.2f Sieverts%n",
                    node.getName(), node.getType(), data.getTemperature(), data.getPressure(),
                    data.getHumidity(), data.getRadiation());
        }
    }
}